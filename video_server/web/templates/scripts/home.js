$(document).ready(function() {
	...
	$("#uploadform").on('submit', function(e) {
		e.preventDefault()
	  	...
	  	createVideo(vname, function(res, err) {
			...
			$.ajax({
				url : 'http://' + window.location.hostname + ':10003/upload/' + obj['id'],
				//url:'http://127.0.0.1:10003/upload/dbibi',
				type : 'POST',
				data : formData,
				//headers: {'Access-Control-Allow-Origin': 'http://127.0.0.1:10002'},
				crossDomain: true,
    ...
    $(".video-item").click(function () {
  	    var url = 'http://' + window.location.hostname + ':10002/videos/'+ this.id
...
function registerUser(callback) {
	...
	var pwd = $("#pwd").val();
	var apiUrl = window.location.hostname + ':10003/api';
	...
	var dat = {
		'url': 'http://'+ window.location.hostname + ':10000/user',
		'method': 'POST',
...
function signinUser(callback) {
	...
	var dat = {
		'url': 'http://'+ window.location.hostname + ':10000/user/' + username,
		'method': 'POST',
		'req_body': JSON.stringify(reqBody)
	};
...
function getUserId(callback) {
	var dat = {
		'url': 'http://' + window.location.hostname + ':10000/user/' + uname,
		'method': 'GET'
	};
...
function createVideo(vname, callback) {
	...
	var dat = {
		'url': 'http://' + window.location.hostname + ':10000/user/' + uname + '/videos',
		'method': 'POST',
		'req_body': JSON.stringify(reqBody)
	};
...
function deleteVideo(vid, callback) {
  var dat = {
    'url': 'http://' + window.location.hostname + ':10000/user/' + uname + '/videos/' + vid,
    'method': 'DELETE',
    'req_body': ''
  };
...
function postComment(vid, content, callback) {
 var reqBody = {
  ...
  var dat = {
    'url': 'http://' + window.location.hostname + ':10000/videos/' + vid + '/comments',
    'method': 'POST',
    'req_body': JSON.stringify(reqBody)
  };
