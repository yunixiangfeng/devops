...
func AddNewVideo(aid int, name string) (*defs.VideoInfo, error) {
	//create uuid
	vid, err := utils.NewUUID() // 返回的是在服务器的文件名（唯一id）
	if err != nil {
		return nil, err
	}
	t := time.Now()
...
func ListVideoInfo(uname string, from, to int) ([]*defs.VideoInfo, error) {
	...
	/*
		        stmtOut, err:=dbConn.Prepare(`SELECT video_info.id, video_info.author_id, video_info.name, video_info.display_ctime FROM video_info
		118                 INNER JOIN users ON video_info.author_id = users.id
		119                 WHERE users.login_name=?// AND video_info.create_time > FROM_UNIXTIME(?) AND video_info.create_time<=FROM_UNIXTIME(?)
		120                 OREDER BY video_info.create_time DESC`)
